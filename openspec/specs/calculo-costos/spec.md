...
El cálculo MUST convertir la unidad del ingrediente en la receta a la unidad base del ingrediente usando `conversiones_unidades`. Si no hay conversión, asume 1:1.
...
#### Scenario: Conversión de unidades

- GIVEN una receta que usa 1000 gramos de harina
- AND `ingrediente_proveedor.precio_actual` está en kilogramo (precio por kg)
- WHEN se invoca `calcular_costo_receta(receta_id)`
- THEN convierte 1000g → 1kg usando `conversiones_unidades`
- AND el costo es `1 × precio_por_kg`
