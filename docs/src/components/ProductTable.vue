<template>
  <div class="bg-white rounded-lg shadow-md overflow-hidden">
    ...
    <div v-else-if="error" class="px-6 py-4 bg-red-50 border-l-4 border-red-500 text-red-700">
      {{ error }}
      <button @click="clearError" class="ml-2 text-indigo-600 hover:text-indigo-800 underline">Dismiss</button>
    </div>
          ...
          >
            ...
            <td class="px-6 py-4 whitespace-nowrap text-sm text-right space-x-2">
              <button 
                ...
                class="px-3 py-1 text-sm font-medium text-indigo-600 hover:text-indigo-900"
              >
                Editar
              </button>
              <button 
                @click="deleteProduct(product.id)" 
                class="px-3 py-1 text-sm font-medium text-red-600 hover:text-red-900"
              >
                Eliminar
              </button>
            </td>
...
export default {
  ...
  setup() {
    ...
    // Handle product saved (create/update)
    ...
    const exportToExcel = async () => {
      try {
        ...
        const worksheetData = productStore.products.map(product => ({
          ID: product.id,
          Código: product.codigo,
          Nombre: product.nombre,
          Descripción: product.descripcion || '',
          Categoría: product.categoria,
          'Sub Categoría': product.subcategoria || '',
          'Unidad de Medida': product.unidadMedida,
          'Precio Costo': Number(product.precioCosto),
          'Precio Venta': Number(product.precioVenta),
          'Stock Actual': product.stockActual,
          'Stock Mínimo': product.stockMinimo,
          'Stock Máximo': product.stockMaximo,
          Activo: product.activo ? 'Sí' : 'No',
          'Fecha Creación': product.fechaCreacion,
          'Fecha Actualización': product.fechaActualizacion
        }))
        ...
        saveAs(new Blob([wbout], {type: 'application/octet-stream'}), 'productos_sias.xlsx')
      } catch (err) {
        error.value = 'Error al exportar a Excel: ' + err.message
      } finally {
        loading.value = false
      }
    }
