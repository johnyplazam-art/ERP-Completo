...
const activeAppSlug = computed(() => {
  const path = router.currentRoute.value.path
  for (const app of appsDisponibles.value) {
    if (path === '/' + app.slug || path.startsWith('/' + app.slug + '/')) {
      return app.slug
    }
  }
  return null
...
const navItems = computed(() => {
  ...
  // 2. Platform admin section (global, visible siempre para platform admin)
      ...
      <div class="border-t border-gray-200 dark:border-gray-800 p-4">
        ...
        >
          ...
          <div v-if="!appStore.sidebarCollapsed" class="ml-3 flex-1 min-w-0">
            ...
            <p v-if="authStore.isPlatformAdmin" class="text-[11px] font-semibold text-amber-600 dark:text-amber-400 truncate tracking-wide uppercase">
              Platform Admin
            </p>
          </div>
        </router-link>
