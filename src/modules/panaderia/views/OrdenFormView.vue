...
const onSubmit = handleSubmit(async (formValues) => {
  try {
    if (isEdit.value) {
      ...
      toast.success(t('produccion.updated'))
    } else {
      await createMutation.mutateAsync(formValues)
      toast.success(t('produccion.created'))
    }
    router.push('/panaderia/produccion')
  } catch (err) {
    toast.error(err.message || t('produccion.saveError'))
  }
})
</script>
...
<template>
  <div>
    <div class="flex items-center mb-6">
      ...
      <h2 class="text-2xl font-bold text-gray-900">{{ isEdit ? t('produccion.editTitle') : t('produccion.createTitle') }}</h2>
    </div>
    ...
    <form v-else @submit="onSubmit" class="max-w-3xl space-y-6">
      ...
      <div class="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
        ...
        <div>
          ...
          <textarea
            ...
            rows="2"
            class="touch-input block w-full rounded-lg border border-gray-300 bg-white text-gray-900 focus:ring-2 focus:ring-primary-500"
            :placeholder="t('produccion.formNotePlaceholder')"
          ></textarea>
        </div>
      </div>
      ...
      <div class="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
        ...
        <div v-if="costoTotalOrden > 0" class="flex justify-end px-3 pt-2 border-t border-gray-200">
          <div class="text-sm font-semibold text-gray-900 tabular-nums">
            {{ t('common.estimatedTotal') }}: $ {{ costoTotalOrden.toLocaleString('es-AR', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) }}
          </div>
        </div>
      </div>
      ...
      >
        <h3 class="text-lg font-semibold text-gray-900 mb-3">{{ t('produccion.formRawMaterials') }}</h3>
        <div v-if="calculandoIng" class="text-sm text-gray-400">
          <i class="pi pi-spin pi-spinner mr-1"></i> {{ t('common.calculating') }}
        </div>
        <div v-else class="overflow-x-auto">
          <table class="w-full text-sm">
            <thead>
              <tr class="text-left text-gray-500 font-medium">
                <th class="pb-2 pr-4">{{ t('produccion.formIngredient') }}</th>
                <th class="pb-2 text-right">{{ t('produccion.formTotalQty') }}</th>
              </tr>
            </thead>
      ...
      <div class="flex justify-end gap-3">
        ...
        >
          ...
          {{ isPending ? t('crud.saving') : (isEdit ? t('produccion.saveChanges') : t('produccion.createOrder')) }}
        </button>
      </div>
    </form>
  </div>
